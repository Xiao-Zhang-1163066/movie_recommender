...
export function WatchlistCard({ item, activeTab, onStatusChange, onRating, onDelete }: Props) {
    ...
    >
      ...
      >
        {item.movie.posterUrl ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-xl" style={{ background: "var(--surface-3)" }}>
            🎬
          </div>
        )}
      </div>
      ...
      <div className="flex flex-1 items-start gap-3 pt-1 min-w-0">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {item.movie.voteAverage != null && (
              <span
                ...
                style={{ background: "rgba(0,0,0,0.35)", color: "var(--lime)" }}
              >
          ...
          {activeTab === "watchlist" && (
            <div className="mt-3 flex items-center gap-2">
              <span className="text-xs" style={{ color: "var(--text-2)" }}>Status</span>
              <select
                value={item.status}
                onChange={(e) => onStatusChange(item, e.target.value as WatchlistStatus)}
                className="text-xs rounded-lg px-2 py-1 font-semibold"
                style={{ background: "var(--chip-bg)", color: "var(--text-2)", border: "none" }}
              >
